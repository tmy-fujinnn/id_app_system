 ...
 *    - text fields, combo boxes, buttons, nav bars, form sections, and data tables.
 */
...
public final class UIFactory {
    ...
    public static JButton createPrimaryButton(String text) {
        ...
        btn.addMouseListener(new MouseAdapter() {
            public void mouseEntered(MouseEvent e) { btn.setBackground(Constants.C_PRIMARY_DARK); }
            public void mouseExited (MouseEvent e) { btn.setBackground(Constants.C_PRIMARY); }
        });
        return btn;
    }
    ...
    public static JPanel buildMiniNav(String pageTitle, JFrame frame) {
        ...
        JPanel left  = new JPanel(new FlowLayout(FlowLayout.LEFT,  0, 16)); left.setOpaque(false);  left.add(title);
        JPanel right = new JPanel(new FlowLayout(FlowLayout.RIGHT, 0, 14)); right.setOpaque(false); right.add(back);
    ...
    public static void autoSizeColumns(JTable table,
        ...
        for (int col = 0; col < colCount; col++) {
            int w = Math.max(80, hFm.stringWidth(model.getColumnName(col)) + 28);
            for (int row = 0; row < model.getRowCount(); row++) {
                Object val = model.getValueAt(row, col);
                if (val != null) w = Math.max(w, cFm.stringWidth(val.toString()) + 28);
            }
            widths[col] = w;
            total += w;
        }
